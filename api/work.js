...
let works = [
  {
    ...
    description:
      'Redesigned personal portfolio website with modern UI/UX practices',
  ...
  },
  {
    ...
    description:
      'Initial consultation with client to gather project requirements',
    category: 'Meeting',
    hoursSpent: 2,
    date: '2024-01-14',
    status: 'Completed',
...
export default function handler(req, res) {
  ...
  switch (method) {
    case 'GET': {
      ...
      if (req.query.category) {
        filteredWorks = filteredWorks.filter(
          (w) => w.category === req.query.category
        )
      }

      if (req.query.status) {
        filteredWorks = filteredWorks.filter(
          (w) => w.status === req.query.status
        )
      }
    ...
    case 'POST': {
      ...
      const { title, description, category, hoursSpent, date, status, tags } =
        req.body
